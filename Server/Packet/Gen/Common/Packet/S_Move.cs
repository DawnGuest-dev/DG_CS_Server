// <auto-generated>
//  automatically generated by the FlatBuffers compiler, do not modify
// </auto-generated>

namespace Common.Packet
{

using global::System;
using global::System.Collections.Generic;
using global::Google.FlatBuffers;

public struct S_Move : IFlatbufferObject
{
  private Table __p;
  public ByteBuffer ByteBuffer { get { return __p.bb; } }
  public static void ValidateVersion() { FlatBufferConstants.FLATBUFFERS_25_2_10(); }
  public static S_Move GetRootAsS_Move(ByteBuffer _bb) { return GetRootAsS_Move(_bb, new S_Move()); }
  public static S_Move GetRootAsS_Move(ByteBuffer _bb, S_Move obj) { return (obj.__assign(_bb.GetInt(_bb.Position) + _bb.Position, _bb)); }
  public void __init(int _i, ByteBuffer _bb) { __p = new Table(_i, _bb); }
  public S_Move __assign(int _i, ByteBuffer _bb) { __init(_i, _bb); return this; }

  public int PlayerId { get { int o = __p.__offset(4); return o != 0 ? __p.bb.GetInt(o + __p.bb_pos) : (int)0; } }
  public Common.Packet.Vec3? Pos { get { int o = __p.__offset(6); return o != 0 ? (Common.Packet.Vec3?)(new Common.Packet.Vec3()).__assign(o + __p.bb_pos, __p.bb) : null; } }

  public static Offset<Common.Packet.S_Move> CreateS_Move(FlatBufferBuilder builder,
      int player_id = 0,
      Common.Packet.Vec3T pos = null) {
    builder.StartTable(2);
    S_Move.AddPos(builder, Common.Packet.Vec3.Pack(builder, pos));
    S_Move.AddPlayerId(builder, player_id);
    return S_Move.EndS_Move(builder);
  }

  public static void StartS_Move(FlatBufferBuilder builder) { builder.StartTable(2); }
  public static void AddPlayerId(FlatBufferBuilder builder, int playerId) { builder.AddInt(0, playerId, 0); }
  public static void AddPos(FlatBufferBuilder builder, Offset<Common.Packet.Vec3> posOffset) { builder.AddStruct(1, posOffset.Value, 0); }
  public static Offset<Common.Packet.S_Move> EndS_Move(FlatBufferBuilder builder) {
    int o = builder.EndTable();
    return new Offset<Common.Packet.S_Move>(o);
  }
  public S_MoveT UnPack() {
    var _o = new S_MoveT();
    this.UnPackTo(_o);
    return _o;
  }
  public void UnPackTo(S_MoveT _o) {
    _o.PlayerId = this.PlayerId;
    _o.Pos = this.Pos.HasValue ? this.Pos.Value.UnPack() : null;
  }
  public static Offset<Common.Packet.S_Move> Pack(FlatBufferBuilder builder, S_MoveT _o) {
    if (_o == null) return default(Offset<Common.Packet.S_Move>);
    return CreateS_Move(
      builder,
      _o.PlayerId,
      _o.Pos);
  }
}

public class S_MoveT
{
  public int PlayerId { get; set; }
  public Common.Packet.Vec3T Pos { get; set; }

  public S_MoveT() {
    this.PlayerId = 0;
    this.Pos = new Common.Packet.Vec3T();
  }
}


static public class S_MoveVerify
{
  static public bool Verify(Google.FlatBuffers.Verifier verifier, uint tablePos)
  {
    return verifier.VerifyTableStart(tablePos)
      && verifier.VerifyField(tablePos, 4 /*PlayerId*/, 4 /*int*/, 4, false)
      && verifier.VerifyField(tablePos, 6 /*Pos*/, 12 /*Common.Packet.Vec3*/, 4, false)
      && verifier.VerifyTableEnd(tablePos);
  }
}

}
